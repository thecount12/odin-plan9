...
Local TCP self-tests: `announce` with `tcp!*!port`, then dial via `netmkaddr(nil, "tcp", port)` — not `tcp!$sysname!port`, which often hangs while the listener waits on `accept`.
...
| 15 | cgen + build.rc (C emit bootstrap) | started |
